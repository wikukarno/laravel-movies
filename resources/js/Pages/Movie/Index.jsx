...
export default function Index({ auth, movie }) {
    ...
    const filterItem = movie.filter((item) =>
        item.name.toLowerCase().includes(filterText.toLowerCase())
    );

    const [confirmingMovieDeletion, setConfirmingMovieDeletion] =
        useState(false);
    ...
    const deleteGenre = async (e) => {
        e.preventDefault();
        try {
            await destroy(route("movie.destroy", dataMovie), {
                preserveScroll: true,
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="flex gap-1">
                    <Link href={route("movie.edit", row.id)}>
